...
import com.SpringBoot1.exception.StudentException;
...
//		}
		return repo.findById(id).orElseThrow(()->new StudentException("Student","ID",id));
		//return repo.findById(id).orElseThrow( ()->new ResoureNotFound("Student", "ID", id));
	}

	@Override
	public Student updateStudentById(Student std, Long id) {
		Student existStudent=repo.findById(id).orElseThrow( () -> new StudentException("Student", "Id", id));
	...
	public void deleteStudentById(Long id) {
		Student existingStudent=repo.findById(id).orElseThrow(() -> new StudentException("Student", "Id", id));
